...
local function client_info()
    ...
    for i = 1, #inf do
        v = v .. string.format('%2s: <span color="%s">%-20s</span> = <span color="%s">%s</span>\n',
			       ...
			       beautiful.fg_urgent, tostring(c[inf[i]]))
    end
...
function dbg(...)
    ...
    local text = "<span color='" .. colors.header ..
        "'>dbg</span> <span color='" .. colors.count .. "'>#" .. num .. "</span>"
    local depth = 2
    local clients = 0
    ...
    if clients ~= num then
        naughty.notify{text = text, timeout = 0,
                       hover_timeout = 2, screen = screen.count()}
    end
end
